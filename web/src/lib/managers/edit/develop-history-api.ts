...
export function listVersions(assetId: string): Promise<DevelopHistoryList> {
  return apiRequest<DevelopHistoryList>(`${BASE}/assets/${assetId}/develop-history`);
}

/** Get current version with full state */
export function getCurrentVersion(assetId: string): Promise<DevelopHistoryVersion | null> {
  return apiRequest<DevelopHistoryVersion | null>(`${BASE}/assets/${assetId}/develop-history/current`);
}
...
export function getVersion(assetId: string, version: number): Promise<DevelopHistoryVersion> {
  return apiRequest<DevelopHistoryVersion>(`${BASE}/assets/${assetId}/develop-history/${version}`);
}
...
export function saveVersion(
  ...
  return apiRequest<DevelopHistoryVersion>(`${BASE}/assets/${assetId}/develop-history`, {
    method: 'POST',
    body: JSON.stringify({
      state,
      label: options?.label ?? '',
...
export function restoreVersion(
  ...
  return apiRequest<DevelopHistoryVersion>(
    `${BASE}/assets/${assetId}/develop-history/${version}/restore`,
    {
      method: 'POST',
...
export function updateLabel(assetId: string, version: number, label: string): Promise<void> {
  return apiRequest<void>(`${BASE}/assets/${assetId}/develop-history/${version}/label`, {
    method: 'PUT',
    body: JSON.stringify({ label }),
  });
}
...
export function deleteVersion(assetId: string, version: number): Promise<void> {
  return apiRequest<void>(`${BASE}/assets/${assetId}/develop-history/${version}`, {
    method: 'DELETE',
  });
}
...
export function getThumbnailUrl(assetId: string, version: number): string {
  return `${BASE}/assets/${assetId}/develop-history/${version}/thumbnail`;
}
